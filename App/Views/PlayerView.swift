...
private struct PlaybackQuality: View {
    @Environment(AppStore.self) private var store
    ...
    var body: some View {
        Group {
            ...
            else if let quality = store.player.resource?.quality { Text(AudioQuality(rawValue: quality)?.label ?? quality) }
        }.task(id: store.player.isBuffering) {
            waiting = false
            if store.player.isBuffering { do { try await Task.sleep(for: .milliseconds(300)); waiting = true } catch { } }
        }
    }
}
struct LyricsReadingPosition {
    var trackID: Int64?
    var anchor: Int?
    var following = true
}
struct LyricsView: View {
    @Binding var reading: LyricsReadingPosition
    @Environment(AppStore.self) private var store
    @Environment(\.dismiss) private var dismiss
            ...
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 28) {
                        if loading { DelayedProgress(title: "读取歌词…") }
                        ...
                        ForEach(lines) { line in
                            Button {
                                if let start = line.start { store.player.seek(max(0, start - (store.player.resource?.previewStart ?? 0))); reading.following = true }
                                ...
                                .buttonStyle(.plain).disabled(line.start == nil).id(line.id).accessibilityIdentifier("lyricLine-\(line.id)")
                        }
                    }.scrollTargetLayout().padding(.horizontal, 28).padding(.vertical, 70)
                }.scrollPosition(id: $scrollAnchor, anchor: .center)
                    .onChange(of: scrollAnchor) { _, value in if let value { reading.anchor = value } }
                    .simultaneousGesture(DragGesture(minimumDistance: 12).onChanged { _ in reading.following = false })
                ...
                .task(id: store.player.current?.id) {
                    if reading.trackID != store.player.current?.id { reading = LyricsReadingPosition(trackID: store.player.current?.id) }
                    restoredPosition = false; lines = []; await load()
                }
        }
    }
    ...
    private func load() async {
        guard let id = store.player.current?.id else { return }; loading = lines.isEmpty; error = nil
        let token = UUID(); loadToken = token
        defer { if loadToken == token { loading = false } }
        do { for try await result in store.lyricUpdates(id) { try Task.checkCancellation(); guard id == store.player.current?.id, loadToken == token else { return }; lines = result; loading = false } } catch is CancellationError { } catch { if loadToken == token { self.error = error.localizedDescription } }
    }
}
struct QueueView: View {
    @Binding var anchor: UUID?
    @State private var restoringPosition = true
    @Environment(AppStore.self) private var store
    @Environment(\.dismiss) private var dismiss
    ...
    var body: some View {
        ...
        NavigationStack {
            ...
            List {
                ...
                Section {
                    ForEach(player.queue.upcoming) { entry in
                        ...
                        }.listRowBackground(Palette.background).id(entry.id)
                            .onGeometryChange(for: Bool.self) { geometry in
                                let frame = geometry.frame(in: .named("queueViewport"))
                                return frame.minY >= 0 && frame.minY < frame.height
                            } action: { _, atTop in
                                if atTop && !restoringPosition { anchor = entry.id }
                            }
            ...
            }.coordinateSpace(name: "queueViewport").cabinetList().navigationTitle("播放队列").navigationBarTitleDisplayMode(.inline)
                .task {
                    if let anchor, player.queue.upcoming.contains(where: { $0.id == anchor }) { proxy.scrollTo(anchor, anchor: .top) }
                    await Task.yield(); restoringPosition = false
                }
