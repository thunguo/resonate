...
struct SearchView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                VStack(alignment: .leading, spacing: 10) { Eyebrow(text: "寻找一个声音"); Text("搜索").font(.largeTitle.weight(.medium)) }
                ...
                results
                if isSearching { ProgressView("正在寻找…").frame(maxWidth: .infinity).padding(24) }
                if result.hasMore && !isSearching { Button("加载更多") { search(more: true) }.frame(maxWidth: .infinity, minHeight: 44) }
                if didSearch && !isSearching && error == nil && result.tracks.isEmpty && result.albums.isEmpty && result.artists.isEmpty && result.playlists.isEmpty { EmptyState(symbol: "magnifyingglass", title: "还没有找到", detail: "换一个歌名或音乐人试试，也可以用一句话描述想听的音乐。") }
            }.padding(20)
        }.cabinetBackground().toolbar(.hidden, for: .navigationBar).onDisappear { task?.cancel(); isSearching = false }
    }
    ...
    @ViewBuilder private var results: some View {
        LazyVStack(spacing: 14) {
            switch kind {
            case .tracks: ForEach(Array(result.tracks.enumerated()), id: \.element.id) { index, track in TrackRow(track: track) { store.player.play(result.tracks, at: index, origin: .search) } }
    ...
    private func search(more: Bool = false) {
        ...
        if !more { offset = 0; result = .init() }
        let pageOffset = more ? offset + 30 : 0
        let selectedKind = kind
        task = Task {
            do {
                let response = try await store.music.search(text, kind: selectedKind, offset: pageOffset)
                try Task.checkCancellation(); guard generation == token else { return }
                if more { result.tracks += response.tracks; result.albums += response.albums; result.artists += response.artists; result.playlists += response.playlists; result.hasMore = response.hasMore }
                else { result = response }
                offset = pageOffset; didSearch = true; store.recordSearch(text)
