...
0.3.6 为编排增加可选的 `seedTrack` 与 `feedback` 字段，旧结果仍可解码。起点固定为首曲并纳入时长与收藏比例；候选复用相似歌曲缓存和本机音乐库，关联仅表达相同音乐人、专辑与服务返回的相似结果，起点资料不会被当作其他歌曲的事实。

局部替换只调用一次模型，返回原歌曲 ID 到候选歌曲 ID 的映射。本地校验替换范围、唯一性、完整播放资格、反馈排除项和整组收藏比例；未选曲目与位置由本地保留，模型无法重写。当前项、手动固定项、起点及身份重复的项目不可替换。完整歌曲导致的时长变化直接展示。

预览不保存新结果，也不修改主队列；明确采用后一次写入新旧两份编排，原结果和远端保存状态保留；即使原结果位于最近 10 份的末尾，也不会被这次采用操作挤出。播放／应用队列仍为独立动作，队列变化时沿用签名检查。反馈按歌曲 ID 合并，仅随当前编排及其后续调整保存，不推断长期偏好；移除反馈会撤销后续候选排除。

调试界面可使用 `--preview --ai-result --editing-test`；`--editing-failure` 注入模型限流，`--editing-slow` 用于取消验证。测试替换音乐资格与模型协议使用独立内存接口，不读取密钥、不发送真实模型请求。真实厂商选歌效果与协议回归分别验收。
