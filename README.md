...
余音是一款面向 iPhone 的网易云第三方音乐客户端。从熟悉的收藏开始，让听过的音乐容易找回，让新的音乐自然进入；用完整封面、清晰的文字和适度的留白，把注意力留给音乐。
...
<details>
<summary>找回听过的音乐</summary>

<table>
  <tr>
    <td align="center"><img src="Screenshots/recent-listening.png" width="250" alt="按日期浏览最近听过的歌曲"><br><sub>最近听过</sub></td>
    <td align="center"><img src="Screenshots/arrangement-library.png" width="250" alt="保留与重听自己的音乐编排"><br><sub>我的编排</sub></td>
  </tr>
</table>
...
<details>
<summary>从一首歌出发，逐首调整</summary>

<table>
  <tr>
    <td align="center"><img src="Screenshots/along-song.png" width="250" alt="以一首歌为起点编排音乐"><br><sub>沿着这首听</sub></td>
    <td align="center"><img src="Screenshots/arrangement-revision.png" width="250" alt="采用前对照原曲和替代歌曲"><br><sub>局部替换</sub></td>
  </tr>
</table>
...
- **从收藏开始听**：继续上次的队列和进度，从久未播放的收藏中挑出最多 12 首，重听一小段音乐；可手动换一组，正在播放的队列保持原样。
- **快速回到音乐**：已缓存的内容先从设备呈现，过期资料在后台更新；搜索先查收藏，再补充远程结果，各分类分别保留结果和分页，失败时可重试当前页。刷新时保留已有内容和封面，智能预缓存会根据网络、电量和设备状态调整。
- **找回最近听过的歌**：按日期浏览本机最近听过的 100 首歌曲，搜索歌名、音乐人或专辑；单曲播放保留原队列，也可以选择“从这里继续听”。支持移除单条和清空记录。
- **留下喜欢的编排**：在“我的编排”浏览、改名和重听已有结果；最近保留 10 份，主动保留的编排不受此数量限制。打开旧结果无需再次生成，同一需求也能明确新建一组。保存到网易云歌单单独操作。
...
- **沿着这首听**：从歌曲或播放器的更多菜单出发，以当前选择的歌曲为起点，结合收藏、相同音乐人与专辑、相似歌曲继续探索。先预览，再决定播放；已有音乐照常继续。
- **只调整需要改变的部分**：勾选歌曲或选择“只替换这首”，先试听替换方案，再采用为新的编排；其余歌曲和顺序保持，原编排及其远端保存状态保留。
- **明确这次的偏好**：记录“最近听得太多”“这次少些人声”等反馈，只用于当前编排和后续调整，可随时移除，不自动改写长期偏好或播放队列。情绪与人声资料不足时按偏好匹配，不声称分析过音频。
...
当前为 **0.3.6 开发版本**。离线下载尚未开放；小组件、快捷指令及 CarPlay 的能力边界见 [开发说明](Docs/DEVELOPMENT.md#系统能力)。
